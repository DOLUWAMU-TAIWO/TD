...
void loop() {
  ...
  int moistureLevel = readSoilMoisture();

  // Publish moisture level to MQTT broker
  mqttClient.publish(moistureTopic, String(moistureLevel).c_str());

  // Check moisture level and control relay
  if (moistureLevel > 50) {
    mqttClient.publish(acuterTopic, "ON");
  } else {
    mqttClient.publish(acuterTopic, "OFF");
  }
